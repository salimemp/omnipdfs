...
export default function DocumentSummarizer({ document, isDark = true }) {
  ...
  const generateSummary = async () => {
    ...
    try {
      const result = await base44.functions.invoke('aiAssistant', {
        task: 'summarize',
        input: {
          text: document?.content || 'Sample document content',
  ...
  const generateTags = async () => {
    ...
    try {
      const result = await base44.functions.invoke('aiAssistant', {
        task: 'tag',
        input: {
          text: document?.content || summary?.main_summary || 'Sample content',
          fileUrl: document?.file_url
        }
      });
